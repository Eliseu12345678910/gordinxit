...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!chatSnapshot.exists) {
      return NextResponse.json({ error: 'Login Gordin du Xit nao encontrado.' }, { status: 404 })
    }
    ...
    if (!isAccountOwner && !isParticipant) {
      return NextResponse.json({ error: 'Voce nao pode alterar este acesso.' }, { status: 403 })
    }
